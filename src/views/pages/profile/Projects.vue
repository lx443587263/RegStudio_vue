  ...
  <div class="py-1 container-fluid">
    <section class="py-3">
      <div class="mt-2 row mt-lg-4">
        ...
        <div v-for="item in allCategoryListVuex" :key="item">
          <complex-project-card
            :title="item.category"
            @dblclick="enterIPList(item.category)"
          />
        </div>
        ...
        <div>
          <placeholder-card :title="{ text: '新建IP', variant: 'h5' }" @click="toNewProject"/>
        </div>
      </div>
    </section>
  </div>
...
// import {getCategoryList} from '@/http/api/ip';
...
export default {
  ...
  data() {
    return {
      showMenu: false,
      allCategoryList:[],
      // allCategoryListVuex:this.$store.state.IP.allCategoryListVuex
    };
  },
  //关联vuex获取数据
  computed:{
  ...
  beforeMount() {
    ...
    // localStorage.setItem("allCategoryListVuex",JSON.stringify(this.allCategoryListVuex))
    // JSON.parse(localStorage.getItem("allCategoryListVuex"))
  ...
  methods:{
    ...
    enterIPList(data){
      // console.log(data)
      getCategoryListApi(data).then(async (res)=>{        
